...
async def sql_get_user_info(
    ...
    try:
        stmt = select(
            ...
            func.count(UserFavorite.card_id).label('favorites_count')
        ).select_from(User) \
        ...
        user_data = user_data_result.first() or ("none", "none", "none", "none", "none")
...
async def sql_update_and_get_stats_user(
    ...
    try:
        ...
        user_stats = user_stats_result.first() or ("none", "none")
        
        await session.commit()
        
        return (telegram_username, telegram_first_name, telegram_last_name, *user_stats)
